...
DO $$
BEGIN
  -- Enable RLS on invoices if table exists and RLS is not enabled
  ...
  -- Enable RLS on user_sessions if table exists and RLS is not enabled
  ...
  -- Enable RLS on system_settings if table exists and RLS is not enabled
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'system_settings') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'system_settings' AND relrowsecurity = true) THEN
      ALTER TABLE system_settings ENABLE ROW LEVEL SECURITY;
    END IF;
  END IF;

  -- Enable RLS on activity_logs if table exists and RLS is not enabled
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'activity_logs') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'activity_logs' AND relrowsecurity = true) THEN
      ALTER TABLE activity_logs ENABLE ROW LEVEL SECURITY;
    END IF;
  END IF;

  -- Enable RLS on email_templates if table exists and RLS is not enabled
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'email_templates') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'email_templates' AND relrowsecurity = true) THEN
      ALTER TABLE email_templates ENABLE ROW LEVEL SECURITY;
    END IF;
  END IF;

  -- Enable RLS on file_downloads if table exists and RLS is not enabled
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'file_downloads') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'file_downloads' AND relrowsecurity = true) THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'invoices' AND policyname = 'Users can read own invoices') THEN
    EXECUTE 'CREATE POLICY "Users can read own invoices" ON invoices FOR SELECT TO authenticated USING (user_id = auth.uid())';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'invoices' AND policyname = 'Admins can manage all invoices') THEN
    EXECUTE 'CREATE POLICY "Admins can manage all invoices" ON invoices FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true))';
  END IF;

  -- User sessions policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'user_sessions' AND policyname = 'Users can read own sessions') THEN
    EXECUTE 'CREATE POLICY "Users can read own sessions" ON user_sessions FOR SELECT TO authenticated USING (user_id = auth.uid())';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'user_sessions' AND policyname = 'Admins can read all sessions') THEN
    EXECUTE 'CREATE POLICY "Admins can read all sessions" ON user_sessions FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true))';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'system_settings' AND policyname = 'Admins can manage system settings') THEN
    EXECUTE 'CREATE POLICY "Admins can manage system settings" ON system_settings FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true))';
  END IF;

  -- Activity logs policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'activity_logs' AND policyname = 'Admins can read activity logs') THEN
    EXECUTE 'CREATE POLICY "Admins can read activity logs" ON activity_logs FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true))';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'activity_logs' AND policyname = 'System can insert activity logs') THEN
    EXECUTE 'CREATE POLICY "System can insert activity logs" ON activity_logs FOR INSERT TO authenticated WITH CHECK (true)';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'email_templates' AND policyname = 'Admins can manage email templates') THEN
    EXECUTE 'CREATE POLICY "Admins can manage email templates" ON email_templates FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true))';
  END IF;

  -- File downloads policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'file_downloads' AND policyname = 'Users can read own downloads') THEN
    EXECUTE 'CREATE POLICY "Users can read own downloads" ON file_downloads FOR SELECT TO authenticated USING (user_id = auth.uid())';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'file_downloads' AND policyname = 'Users can insert own downloads') THEN
    EXECUTE 'CREATE POLICY "Users can insert own downloads" ON file_downloads FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid())';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'file_downloads' AND policyname = 'Admins can read all downloads') THEN
    EXECUTE 'CREATE POLICY "Admins can read all downloads" ON file_downloads FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true))';
  END IF;
END $$;

-- Add updated_at triggers for new tables (with existence checks)
DO $$
BEGIN
  -- Check if trigger exists before creating
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_invoices_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_invoices_updated_at BEFORE UPDATE ON invoices FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_email_templates_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_email_templates_updated_at BEFORE UPDATE ON email_templates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()';
  END IF;
END $$;
...
EXCEPTION
  WHEN OTHERS THEN
    -- Silently ignore errors in activity logging to prevent breaking main operations
    NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
